<template>
  <div class="home">
    ...
    <div>{{ arr2 }}</div>
    <h1>加油，上进！</h1>
    <div>一个月后：{{ futureTime(1) }}</div>
    <div>三个月后：{{ futureTime(3) }}</div>
    <div>六个月后：{{ futureTime(6) }}</div>
    <div>一年后：{{ futureTime(12) }}</div>
    <div>两年后：{{ futureTime(24) }}</div>
    <div>两年后：{{ futureTime(120) }}</div>
    <el-autocomplete
      v-model="address"
      value-key="value"
