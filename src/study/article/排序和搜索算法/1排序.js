...
function ArrayList() {
  ...
  this.insertSort = function() {
    const len = arr.length // arr = [3, 5, 1, 4, 2]为例
    let num
    let tmp
  ...
  function mergeRecurve(arr) {
    ...
    const right = arr.slice(mid, len) // 到这里把数组一分为二
  ...
  function partition(arr, left, right) {
    const point = arr[Math.floor((left + right) / 2)]
    let i = left
    let j = right // 双指针
